...
| Full automated suite | **3381 passed / 0 failed / 0 skipped** across **153 files** + **151 E2E** across **80 spec files** = **3532 green** (measured 2026-09-16) |
...
And an **empty cut-set list over a state machine**: a state machine passed as
`--element` is answered with a measurement rather than a tree — how many
failure-mode flags and `#exceptional` states it carries, which is what a
behavioural fault tree would inject and aim at — and exits 2 under the row that
names *a state machine whose failure modes this command found none of* (or, over
a machine carrying a flag this build does not inject, *found and does not
inject* — the same row). Nothing is enumerated over it and no absence is
claimed. And the route measures exactly the machines the census counts — the
roots `reach` walks: a state inside a machine is refused with the machine named,
and a state definition that owns no transition is refused as not one of this
tool's machines, so one payload never carries a machine answer beside
`machines: 0` and a tagged hazard is never counted against itself as `0`. The
`--json` payload carries a
file-wide census of the same two things (`faultTree.behaviouralLane`), which is
the measurement a behavioural fault tree is held behind; it reads **0** on every
shipped example and every verification fixture, and a census that finds nothing
is the result rather than a failure.
...
that consumed a trigger carries the environment sentence, which names the
triggers that run consumed: the walk offers every trigger the machine names at
every configuration, so a witness that consumes `abort` is a claim about that
environment and not about one that withholds `abort` — a function of the
triggers since commit 24 of the model-checking plan, never a constant with an
example trigger inside it. A run that crossed
...
**A refutation says whether it is one run or every run, and the word is
gated.** Every `fail` a bad-prefix search produces carries an `every run?`
line — `modality` on `--json` — with one of three values. `potential` claims
that some maximal run of the MACHINE avoids the violation, and names it: a
cycle that never enters the violating configuration, or a configuration of
the retained relation with no way out; it is register row W3, a maximality
witness, and it stands only where `walkIsExact` holds — on `FlightModes` it
names the cycle `manual → autonomous → manual`, which is the simulator's own
tie-break read the other way. `guaranteed` claims that every maximal run of
the machine reaches the violation; it is register row A8, an absence claim
(no avoiding run), and it reads the same gate. `not decided` is the word for
everything else, with its reason: a walk not seen whole, a stateful monitor or
a step atom (the product does not collapse onto the configuration graph), an
expression atom (the store is not retained), a named trigger, an `after(n)`
dwell, or a guard the walk consulted and could not decide — on the latch
machine the naive answer is `guaranteed`, about a design that sits in
`nominal` forever when nobody sends `unlatch`, and the line says `not
decided` instead. None of the three moves a count, a code or the exit code:
the row is a `fail` with a witness before the line and after it. What none of
them claims: nothing about the interpreter (`simulate` takes the first
declared branch and may never produce the run `potential` names — the
simulator sentence prints beside it wherever the machine has a choice point),
nothing about an environment, and nothing about time.
...
# Full unit + integration + conformance suite (3381 pass / 0 skip, 153 files)
...
# End-to-end (151 tests across 80 spec files)
npm run test:e2e
```
