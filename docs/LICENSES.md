...
| **z3** theorem prover, via the `z3-solver` npm package (WASM build) | <https://github.com/Z3Prover/z3> | **MIT** | **OPTIONAL** runtime dependency of the verification lane, loaded through a dynamic import by `src/semantics/smt/z3-bridge.ts` and excluded from the browser bundle (`vite.config.ts`). Nothing is copied from it: this project writes SMT-LIB2 and reads back `sat` / `unsat` / `unknown`. It is never bundled with a release, and a clone that skips optional dependencies still builds and passes its suites. |
