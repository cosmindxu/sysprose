...
| Capability | Commercial desktop tool | Open-source web tool | **This tool** | Covered by test(s) |
|---|---|---|---|---|
| Model validation + navigable diagnostics | Yes | Yes | **Yes** | `E validation`, `E panels-problems-text` (Validate → `problem-row` → selects element); `U validation.rules` (24 rules, 60 cases) |
...
(Langium, 100% corpus parse); 24-rule validation with navigable diagnostics;
