...
const FAMILIES: Array<{ prefix: string; title: string; blurb: string }> = [
  ...
  { prefix: 'verification/', title: 'Formal verification', blurb: 'The verification lane, and the severity splits it in two. Almost all of these are INFO because none of them is a defect in the model: they say that a relation is outside the fragment the lane encodes, that a contract has nothing to show, that a clause sits somewhere the standard does not admit it, or that a proof would be void rather than absent — each stated rather than left to be inferred from a silence. Exactly two are ERRORS, because they say something about the MODEL: `verification/refuted`, the violation this lane exists to find, and `verification/vacuous-property`, which exists only because `--strict-vacuity` asked for a vacuity to be loud.' },
