...
nothing can satisfy. ${listOf(writing.map((c) => `\`${c.name}\``))} ${writing.length === 1 ? '*writes*' : '*write*'} the
...
|---|---|---|---|
${COMMANDS.map((c) => `| [\`${c.name}\`](#${c.name}) | ${cell(c.question)} | \`${c.payloadKey}\` | ${c.exitContract === 'verify' ? 'judges' : c.exitContract === 'write' ? 'writes' : 'reports'} |`).join('\n')}
