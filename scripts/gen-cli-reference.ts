...
${listOf(behaving.map((c) => `\`${c.name}\``))} ${behaving.length === 1 ? 'judges a *machine*' : 'judge a *machine*'} and ${behaving.length === 1 ? 'carries' : 'carry'} the fifth,
because there is no solver in that lane and no \`--free\` for it either:
${BEHAVIOUR_EXIT_CODES.replace('Exit codes: ', '')}. Each section below states its own
