...
${WRITE_EXIT_CODES.replace('Exit codes: ', '')}. Each section below states its own
...
|---|---|---|---|
${COMMANDS.map((c) => `| [\`${c.name}\`](#${c.name}) | ${cell(c.question)} | \`${c.payloadKey}\` | ${c.exitContract === 'verify' || c.exitContract === 'refine' ? 'judges' : c.exitContract === 'write' ? 'writes' : 'reports'} |`).join('\n')}
