...
| Does this behaviour pattern hold on every reachable configuration, and can this design reach the situation I name? | — no view yet | `npm run sysprose -- check-behaviour model.sysml --element FlightModes --pattern 'pattern=absence, scope=globally, p=state failsafe'` | `behaviourReport` — `src/semantics/mc/patterns.ts` |
...
- [`docs/05-model-checking-literature.md`](docs/05-model-checking-literature.md) — an eleven-volume model-checking library read against this tool: eight candidate features with the page they come from, eighteen topics rejected with the reason, and for each candidate the measurement that would retire it.
- [`docs/06-model-checking-implementation-plan.md`](docs/06-model-checking-implementation-plan.md) — the plan that turns that reading into features: what each new tool may say, the one condition every positive claim passes, the nine lanes and the census that releases each held one.
