...
const CODES = [
  ...
  // looks exactly like a run that worked.
  {
    ...
    source: 'verification',
    severity: 'error',
    when: 'A requirement does not hold with every feature at the value the model binds it to. Both engines file it: the literal one evaluates the relation and reads it as false, and the SMT one finds `A ∧ P ∧ ¬G` satisfiable and confirms the witness in process. `check-behaviour` files the same code for a behavioural property a bad-prefix search REFUTED: a run of the configuration graph that breaks the pattern, printed step by step — and for a `recovery` assertion some reachable configuration cannot discharge: `not recoverable`, naming the SET of configurations from which the named state is unreachable over a relation that is the machine’s own, with no run printed because a set is not a run. It is the one code on a DECIDED row, and the run exits 1.',
  ...
  },
  {
    ...
    source: 'verification',
    severity: 'warning',
    when: 'A reachable configuration has no enabled outgoing transition — no completion transition, and none for any trigger the machine names — and its active leaf is neither marked final nor a `done` node. Reachable means reached by a run of this semantics: a configuration only entered by firing a transition an inner state’s priority always beats is not explored, so nothing is reported there. It is an absence claim about one configuration’s outgoing edges, so it is WITHHELD wherever a guard the walk could not evaluate (`verification/guard-undetermined`) is an edge out of the configuration the row is about — read over its whole active stack, since an edge leaving a composite state is an edge out of every configuration inside it. That guard is an edge out nothing decided. A guard left undecided ELSEWHERE in the machine does not withhold it: the row is a claim about one configuration, and a question asked in another region is not an answer about this one. The report prints a `withheld` line in place of each row that went, so a short list is never read as a clean one. A bound does not withhold it: a bound stops the walk enqueueing successors and takes nothing away from a configuration it already dequeued and offered every input at.',
  ...
  },
  {
    ...
    hint: 'Read which of the two it is off the row: if it names missing names, give those features a value the walk can read (`attribute mode : Integer = 3;`) or drive the machine from a state whose effect assigns them; if it names none, every name resolves and the guard itself is the defect — most often that it is not a predicate. Until then the machine’s unreachable and dead lists are WITHHELD — they are absence claims, and nothing here established the absence — and the report says `undetermined under {…}` rather than `exhaustive`. A no-way-out row goes with them only where this guard is an edge out of the configuration that row is about; a configuration nothing undecided leaves keeps its row, and each row that does go is replaced by a `withheld` line naming why. `check-behaviour` is gated on the same condition and reports the property inconclusive rather than `pass` or `vacuous`. A hidden-choice row still stands where the withheld guard is at or outside the level the choice was read at — two transitions enabled at once is an existential claim, and removing a candidate beside them can only shrink it — but not where the withheld guard is strictly inside it, because that moves which level the choice is read at.',
  },
  {
    code: 'verification/bound-exhausted',
    source: 'verification',
    severity: 'info',
    when: 'A bound stopped the walk: the configuration bound (`--max-configs`), the depth bound, or a chain of completion transitions longer than the 64-step chase budget the interpreter itself runs under. The walk is partial. `check-behaviour` files it for a property whose search found no bad prefix over a graph it did not finish — for one whose scope no explored run opened, because “the antecedent is never met” is a claim of absence a partial walk has not established either — and for a `recovery` over a walk that did not finish, where neither “reachable from every configuration” nor its complement is a claim a partial relation supports: a frontier configuration looks like one nothing leaves.',
  ...
  // make the property inconclusive and exit 2 — never absent, and never false.
  {
    ...
    source: 'verification',
    severity: 'info',
    // The eight names are a HAND COPY of `PATTERNS` (`src/semantics/mc/patterns.ts`),
    ...
    when: 'A behavioural property could not be read as one: a `pattern` outside the catalogue (`absence`, `universality`, `bounded-existence`, `precedence`, `cover`, `recovery`, `existence`, `response`), a `scope` outside its five, a field the pattern needs and did not get, a `bounded-existence` with no `n`, a `--pattern` that is not `key=value`, an expression atom that parses and is not a predicate, so it yielded a number (or nothing) where the walk needed a boolean — or a `p` whose atom kind this pattern cannot observe (`recovery` reads `state` and `node` only: a `trigger` or `fires` atom holds on a step and not at a configuration, and an expression reads a store this walk does not retain, so neither yields a target set to reverse from), or a `recovery` over any scope but `globally`, which this pattern gives no meaning to.',
